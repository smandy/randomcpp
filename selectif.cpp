...
enum class GatewayType { GXA = 0, FIX = 1 };

template <bool cond, typename If, typename Then> struct select_if {
  using type = Then;
};

template <typename If, typename Then> struct select_if<true, If, Then> {
  using type = If;
};
...
template <typename T> struct TemplateAdder {
